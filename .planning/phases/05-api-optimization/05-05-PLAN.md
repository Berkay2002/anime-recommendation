...
<task type="auto">
  <name>Step 1: Add useQueries and replace sequential useEffect hooks</name>
  ...
  <action>
    ...
    3. DO NOT remove old state yet - that will be done in the next task after verifying useQueries works
  ...
  <verify>grep -q "useQueries" frontend/app/anime/[id]/page.tsx && grep -q "queries:\\s*\\[" frontend/app/anime/[id]/page.tsx</verify>
  <done>useQueries hook added with parallel execution of all 3 queries</done>
</task>

<task type="auto">
  <name>Step 2: Remove old state variables and update loading states</name>
  <files>frontend/app/anime/[id]/page.tsx</files>
  <action>
    Remove old state management now that useQueries is working:
    ...
    2. Remove old loading states:
       - Delete: `const { isLoading: loading, setIsLoading } = useLoadingState(150)`
       - Delete: `const { isLoading: recommendationsLoading, setIsLoading: setRecommendationsLoading } = useLoadingState(150)`
       - Delete: `const { isLoading: reviewsLoading, setIsLoading: setReviewsLoading } = useLoadingState(150)`

    3. Remove all `setIsLoading` calls throughout the component (find and remove all instances)

    4. Update error handling:
       - Remove: `const mainError = useErrorHandler()`, `const recommendationsError = useErrorHandler()`, etc.
       - Use `errors` array from useQueries combine result instead

    5. DO NOT update JSX yet - that will be done in the next task

    Note: We're removing state management in a separate task to verify useQueries works first
  </action>
  <verify>grep -c "useState.*anime.*setAnime\|useState.*recommendations.*setRecommendations\|useState.*reviews.*setReviews\|useState.*details.*setDetails\|useState.*loading.*setIsLoading" frontend/app/anime/[id]/page.tsx | grep -q "^0$"</verify>
  <done>Old state variables removed (anime, recommendations, reviews, details, loading states)</done>
</task>

<task type="auto">
  <name>Step 3: Update JSX to use React Query data and error states</name>
  <files>frontend/app/anime/[id]/page.tsx</files>
  <action>
    Update JSX to handle errors array and use React Query states:

    1. Replace old error handling with errors array:
       ```typescript
       {errors.length > 0 && (
         <ErrorMessage
           error={errors[0]}
    ...
    2. Update loading conditions:
       - Replace all `{loading && ...}` with `{isLoading && ...}`
       - Replace all `{recommendationsLoading && ...}` with `{isLoading && ...}`
       - Replace all `{reviewsLoading && ...}` with `{isLoading && ...}`

    3. Update data references:
       - All references to `anime` should now use the `anime` from useQueries result
       - All references to `recommendations` should now use the `recommendations` from useQueries result
       - All references to `reviews` should now use the `reviews` from useQueries result

    4. Remove any remaining old error handling code (catch blocks from useEffect, setError calls, etc.)

    Reference: RESEARCH.md "Example 2: Migrating Existing useEffect to useQuery"

    Key principle: Let React Query handle all data fetching states, remove manual state management
  </action>
  <verify>! grep -q "setError\|setIsLoading\|setAnime\|setRecommendations\|setReviews\|setDetails" frontend/app/anime/[id]/page.tsx && grep -q "errors\\.length" frontend/app/anime/[id]/page.tsx</verify>
  <done>JSX updated to use React Query states (isLoading, errors) and data from useQueries</done>
</task>
...
6. Verify no old useState hooks remain for data fetching state
</verification>
...
8. All old state variables confirmed removed (anime, recommendations, reviews, details, loading states)
</success_criteria>
